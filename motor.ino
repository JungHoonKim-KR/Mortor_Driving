...
void oneByStep(long pulse_target) {
  ...
  if (pulse_target > MaxPulseTarget) {
    ...
    for (int i = 0; i < step; i++) {
      mode0(0x001, 1, 0, tenCMStep, PPS);
      delay((tenCMStep * 1000) / PPS + 10);
      if(PPS<MaxPPS){
        PPS += acceleration;
      }
    ...
    if(remainPulseTarget>0){
      mode0(0x001, 1, 0, remainPulseTarget, PPS);
      delay((remainPulseTarget * 1000) / PPS);
...
void stepByStep(float stepDistance, float objectSize){
  ...
  for (int i = 0; i < stepSize; i++) {
    mode0(0x001, 1, 0, stepD, 200);  // 모터 명령 실행
    delay((stepD * (long)1000)/200+100);
  }

  // 남은 거리가 있을 경우 처리
  if (remainDistance > 0) {
    float remainD = calPulseTarget_float(remainDistance);
    mode0(0x001, 1, 0, remainD, 200);  // 모터 명령 실행
    delay((remainD*1000) / 200);
  }
